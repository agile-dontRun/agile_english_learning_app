...
function renderSavedList(){
    ...
    container.innerHTML=savedOutfits.map((outfit)=>`
        <div class="saved-item ${Number(outfit.is_used)===1?'used':''}" data-id="${outfit.id}" data-name="${escapeHtml(outfit.name||'')}">
            <span>${escapeHtml(outfit.name)}</span>
            ${Number(outfit.is_used)===1?'<span class="used">In Use</span>':''}
            <span class="delete" data-id="${outfit.id}" aria-label="Delete">×</span>
        </div>
    `).join('');
    ...
    document.querySelectorAll('.saved-item').forEach((item)=>{
        item.addEventListener('click',async(event)=>{
            if(event.target.classList.contains('delete')){
                ...
                const confirmed=await askForConfirmation('Delete Outfit',`Do you want to remove "${item.dataset.name}" from My Outfits?`,'Delete');
                if(!confirmed)return;
                await fetch('api/delete_outfit.php',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:`id=${encodeURIComponent(item.dataset.id)}`});
                loadSavedOutfits();
                setStatus('Outfit removed from My Outfits');
                return;
            }
            ...
            if(data.success){
                ...
                selectedLoadedOutfitId=Number(item.dataset.id);
                selectedLoadedOutfitName=item.dataset.name||'';
                selectedLoadedOutfitSignature=buildOutfitSignature(currentOutfit);
                await renderCanvas();
                renderImagesGrid();
                updateSummaryUI();
                await syncCurrentOutfit();
                setStatus(`Loaded outfit: ${item.dataset.name}`);
            }
        });
    });
}
