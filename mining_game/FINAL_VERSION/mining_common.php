<?php
// Import shared coin system functions used by the mining game
require_once __DIR__ . '/coin_common.php';

// Convert user ID into string format for database operations
function mining_user_id_param($userId): string {
    return (string)$userId;
}

// Return all map configuration data used in the mining game
...
// Return achievement reward data used by the backend
...
// Convert a string into a numeric reference ID
// If the value is already numeric, use it directly; otherwise generate one with crc32
...
// Make sure each user has default progress records for every map
function mining_ensure_progress(mysqli $conn, $userId): void {
    ...
    foreach (mining_get_map_catalog() as $mapKey => $map) {
        // The first map is unlocked by default, others start locked
...
// Get the user's progress status for all maps
function mining_get_progress_map(mysqli $conn, $userId): array {
    ...
    // If any map record is missing, fill it with default values
    foreach (mining_get_map_catalog() as $mapKey => $map) {
...
// Prepare initial data for the frontend page
function mining_get_bootstrap(mysqli $conn, $userId): array {
    ...
    // Merge map config data with each map's unlock/clear status
...
// Unlock a map if the player has enough coins and meets the unlock condition
function mining_unlock_map(mysqli $conn, $userId, string $mapKey): array {
    ...
    // If the map is already unlocked, return success directly
    ...
    // Check whether the required previous map has been unlocked
    ...
    try {
        ...
        // Deduct coins and create a transaction record for this unlock
        ...
        // Update map unlock status in mining_map_progress
        $stmt = $conn->prepare("
        ...
        // Also store unlock info in the shared unlock table
        $stmt = $conn->prepare("
...
// Mark a map as cleared after the player finishes it
...
// Update the last played time when the user enters a map
...
// Award coins for collecting ore in single-player mode
function mining_award_single_ore(
    ...
    // Build a unique numeric reference ID based on run ID and item sequence
...
// Award PvP win coins to the winner
function mining_award_pvp_win(mysqli $conn, $userId, string $roomId): array {
    ...
    // Use default reward if no config value is found
    if ($reward <= 0) {
        $reward = 20;
    }
...
// Get all achievement IDs whose rewards have already been claimed by the user
function mining_get_claimed_achievement_ids(mysqli $conn, $userId): array {
    $userIdParam = mining_user_id_param($userId);
    $catalog = mining_get_achievement_catalog();

    // Build a mapping from ref_id back to achievement ID
    $refToId = [];
...
// Claim the reward for a specific achievement
