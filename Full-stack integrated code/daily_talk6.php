...
function render_daily_video_card($video) {
    $json_data = htmlspecialchars(json_encode($video), ENT_QUOTES, 'UTF-8');
    ?>
    <div class="video-card" onclick="openPlayer(<?= $json_data ?>)">
        <img src="<?= $video['cover_url'] ?: 'static/images/default_cover.png' ?>">
        <div class="video-title"><?= htmlspecialchars($video['title']) ?></div>
        <span style="font-size:13px; color:var(--text-light); font-weight:600;">
            Speaker: <?= htmlspecialchars($video['speaker']) ?>
        </span>
    </div>
    <?php
}
...
/**
 * Issue #345 & #346: 渲染模态播放器 HTML 结构
 */
function render_video_modal_component() {
    ?>
    <div id="videoModal" class="modal">
        <div class="modal-content">
            ...
            <h2 id="video-title-display" style="color: white; margin-top: 20px; font-weight: 400; font-family: 'Playfair Display', serif;"></h2>
        </div>
    </div>
    <?php
}
