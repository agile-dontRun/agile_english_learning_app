...
<?php render_oxford_navbar($nickname, $avatar_html); ?>
function render_ai_assistant($nickname) {
    $hour = date('H');
    $greeting = "Greetings";
    ...
    <div class="ai-assistant">
        <div class="chat-bubble">
            <?php echo "{$greeting}, " . htmlspecialchars($nickname); ?>. Ready for an intellectual discovery?
        </div>
    ...
    <?php if ($ted_id === 0): ?>
        <div class="video-grid">
            <?php while ($row = $video_result->fetch_assoc()): ?>
                <?php render_video_card($row, $page); ?>
            <?php endwhile; ?>
        </div>
    <?php else: ?>
        <iframe src="<?php echo format_video_embed_url($talk['video_url']); ?>" allowfullscreen></iframe>
    <?php endif; ?>
</main>

<?php render_ai_assistant($nickname); ?>
