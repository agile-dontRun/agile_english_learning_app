    ...
    function checkAnswers() {
        ...
        // 清除旧提示
            if (user === correct && user !== "") {
                input.style.borderBottom = "2px solid #2d6a4f";
                ...
                wrongDetails.push(`
                    <div style="text-align:left; margin-bottom:8px;">
                        <b>Q${index + 1}</b><br>
                        Your answer: <span style="color:red">${user || "(empty)"}</span><br>
                        Correct answer: <span style="color:green">${correct}</span>
                    </div>
                `);
            }
        });
        let accuracy = correctCount / total;
        ...
        Swal.fire({
            title: title,
            html: `
                <p>Score: <b>${correctCount} / ${total}</b></p>
                <hr>
                ${wrongDetails.length ? wrongDetails.join('') : '<b>Perfect! 🎉</b>'}
            `,
            icon: icon,
            width: 500,
