...
<?php
// 获取口语尝试的总数和平均分
$stat_stmt = $conn->prepare("SELECT COUNT(*) as cnt, AVG(overall_score) as avg FROM user_speaking_attempts WHERE user_id = ?");
// 获取最近 5 条历史记录
$hist_stmt = $conn->prepare("SELECT overall_score, fluency_score, created_at FROM user_speaking_attempts WHERE user_id = ? ORDER BY created_at DESC LIMIT 5");
?>

<div class="stats-grid">
    <div class="stat-card"><h3><?php echo $total_sessions; ?></h3><p>Sessions</p></div>
    <div class="stat-card"><h3><?php echo $avg_score; ?></h3><p>Avg Score</p></div>
</div>
