...
<script>
    const CANTEEN_CORRECT_SOUND = '/y1871.wav';
    const CANTEEN_WRONG_SOUND = '/y1627.wav';
    ...
    const canteenAudioCache = {};

    function playCanteenEffect(path) {
        if (!path) return;
        if (!canteenAudioCache[path]) {
            canteenAudioCache[path] = new Audio(path);
            canteenAudioCache[path].preload = 'auto';
        }
        const audio = canteenAudioCache[path];
        audio.currentTime = 0;
        audio.play().catch(() => {});
    }
    ...
    function onVideoEnded() {
        ...
        playCanteenEffect(data.is_correct ? CANTEEN_CORRECT_SOUND : CANTEEN_WRONG_SOUND);
        ...
        if (data.game_over) {
            gameActive = false;
            const finalRewardAmount = Number(data.reward_amount || 0);
            setTimeout(() => {
                alert(`Game over! You received ${finalRewardAmount} coins this round.`);
            }, 150);
