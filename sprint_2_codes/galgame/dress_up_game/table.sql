...
CREATE TABLE IF NOT EXISTS outfits (
    ...
    UNIQUE KEY uk_user_outfit_name (user_id, name),
...
-- ============================================
-- 4.1 已有数据库升级参考
-- 假设 users 表关联列为 user_id，且类型为 BIGINT
-- 如果 users.user_id 是 BIGINT UNSIGNED，这里也要改成 BIGINT UNSIGNED
-- 已有数据需要先补一个有效的 user_id
-- ============================================
-- ALTER TABLE outfits ADD COLUMN user_id BIGINT NOT NULL DEFAULT 1 AFTER id;
-- ALTER TABLE outfits ADD INDEX idx_user (user_id);
-- ALTER TABLE outfits ADD UNIQUE KEY uk_user_outfit_name (user_id, name);
