document.addEventListener('DOMContentLoaded', function() {
    const STAGE_CORRECT_SOUND = '/y1871.wav';
    const STAGE_WRONG_SOUND = '/y1627.wav';
    ...
    const stageAudioCache = {};

    function playStageEffect(path) {
        if (!path) return;
        if (!stageAudioCache[path]) {
            stageAudioCache[path] = new Audio(path);
            stageAudioCache[path].preload = 'auto';
        }
        const audio = stageAudioCache[path];
        audio.currentTime = 0;
        audio.play().catch(() => {});
    }
    ...
    async function submitAnswer(answerIndex) {
        ...
        try {
            ...
            if (data.is_correct) {
                ...
                playStageEffect(STAGE_CORRECT_SOUND);
                updateScoreDisplay();
                if (feedbackMsgEl) feedbackMsgEl.innerHTML = `✅ Correct!<br>📖 ${data.explanation}`;
            } else {
                playStageEffect(STAGE_WRONG_SOUND);
    ...
    function showResult() {
        ...
        setTimeout(() => {
            alert(`Performance finished! You received ${lastRewardAmount} coins this round.`);
        }, 150);
