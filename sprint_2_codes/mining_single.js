...
function drawPlayerView(p, objects) {
  ...
  ctx.save();
  ctx.translate(pivotX, pivotY);
  ctx.rotate(p.hook.angle - Math.PI / 2); // 仅让绳子和钩子旋转

  const ropeStartY = machineSize / 2 - 20; // 绳子从矿机底部稍微靠内的位置开始
  const ropeEndY = p.hook.length;          // 绳子一直画到钩子当前的长度
