...
const MEMORY_CORRECT_SOUND = '/y1871.wav';
const MEMORY_WRONG_SOUND = '/y1627.wav';
...
const effectAudioCache = {};

function getLookSignature(data) {
    if (!data || !Array.isArray(data.layers) || data.layers.length === 0) {
        return 'empty';
    }

    return data.layers
        .map((layer) => `${layer.layer}:${layer.image_id || 0}`)
        .join('|');
}
...
function renderPlayerLook(stageId, emptyId, data) {
    ...
    const nextSignature = getLookSignature(data);
    if (stage.dataset.lookSignature === nextSignature) {
        return;
    }

    stage.dataset.lookSignature = nextSignature;
...
async function loadLookForUser(userId, stageId, emptyId) {
    ...
    if (lookCache.has(userId)) {
        if (stage) {
            stage.dataset.lookUserId = String(userId);
        }
    ...
    try {
        ...
        const stage = document.getElementById(stageId);
        if (stage && stage.dataset.lookSignature) {
            return;
        }
        renderPlayerLook(stageId, emptyId, null);
    }
...
function playEffectSound(path) {
    if (!path) return;
    if (!effectAudioCache[path]) {
        effectAudioCache[path] = new Audio(path);
        effectAudioCache[path].preload = 'auto';
    }
    const audio = effectAudioCache[path];
    audio.currentTime = 0;
    audio.play().catch(() => {});
}
...
async function flipCard(cardId) {
    ...
    try {
        ...
        if (data.phase === 'second') {
            ...
            if (data.is_match) {
                playEffectSound(MEMORY_CORRECT_SOUND);
                ...
                playEffectSound(MEMORY_WRONG_SOUND);
