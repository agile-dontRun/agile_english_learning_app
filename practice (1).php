...
// 🌟 核心修改：使用子查询 (SELECT ... LIMIT 10) 强制只抽出 10 道题
$stmt2 = $conn->prepare("
...
// 🌟 核心新增：在这里强制只保留 10 道题！
// ==========================================
$keys = array_keys($questions); // 获取所有题目的 ID

if (count($keys) > 10) {
    
    $selected_keys = array_slice($keys, 0, 10); // 强制只切出 10 个
    
    $limited_questions = [];
    foreach ($selected_keys as $key) {
        $limited_questions[$key] = $questions[$key];
    }
    $questions = $limited_questions; // 覆盖原来的大数组，只剩下 10 题
}
// ==========================================
?>
<!DOCTYPE html>
    ...
    <?php foreach ($questions as $q): ?>
        ...
        <p>
            <?php for ($i = 0; $i < count($parts); $i++): ?>
                ...
                <?php
                // 如果已经达到 10 个空，立即停止当前题和后续所有内容的输出
                if ($stopRendering) {
                    break;
                }
