...
<head>
    ...
    /* 2. 增强文字投影：因为去掉了深色滤镜，必须靠投影保护文字不被背景干扰 */
    text-shadow: 0 2px 15px rgba(0, 33, 71, 0.9), 0 0 20px rgba(0,0,0,0.5);
}
...
.hero-mini p {
    ...
    max-width: 820px;
    font-size: 1.05rem;
    opacity: 1; /* 既然去掉了滤镜，建议将透明度设为1，让文字更白 */
}
            ...
            <ul class="navbar-links">
                ...
                <li class="dropdown active">
                    ...
                    <ul class="dropdown-menu">
                        ...
                        <li><a href="reading.php" style="color:var(--oxford-gold)!important; font-weight: bold;">Reading</a></li>
...
function createWordPopup() {
    ...
    div.style.cssText = 'position:absolute; background:white; border:2px solid #40916c; border-radius:16px; padding:12px; max-width:300px; z-index:4001; display:none;';
    document.body.appendChild(div);
    return div;
}
...
async function lookupWord(word) {
    try {
        const response = await fetch(`vocabulary_lookup.php?search=${encodeURIComponent(word)}`, {
...
function getSelectedWord() {
    ...
    const normalizedWord = selectedText
        .replace(/^[^a-zA-Z]+|[^a-zA-Z]+$/g, '')
        .toLowerCase();

    if (!/^[a-zA-Z]+(?:[-'][a-zA-Z]+)*$/.test(normalizedWord)) return null;
...
function getWordFromPoint(x, y) {
    ...
    const word = text.slice(start, end).replace(/^[^A-Za-z]+|[^A-Za-z]+$/g, '').toLowerCase();
...
async function handleWordLookupFromSelection(event) {
    const selected = getSelectedWord() || (event ? getWordFromPoint(event.clientX, event.clientY) : null);
    if (!selected) return;

    const rect = selected.range.getBoundingClientRect();
    const { definition, matchedWord, phonetic, error } = await lookupWord(selected.word);
    const popupWord = matchedWord || selected.word;
    const popupDefinition = error
        ? 'Lookup temporarily unavailable'
        : (phonetic ? `${phonetic}<br>${definition || 'Not found in vocabulary'}` : definition);
    showWordPopup(popupWord, rect.left + window.scrollX, rect.top + window.scrollY, popupDefinition);
}
...
async function storyDblClickHandler(e) {
    const selection = window.getSelection();
    const selectedText = selection.toString().trim();
    if (selectedText && /^[a-zA-Z]+(?:[-\'][a-zA-Z]+)*$/.test(selectedText)) {
        ...
        const { definition, matchedWord, phonetic, error } = await lookupWord(word);
        const popupWord = matchedWord || word;
        const popupDefinition = error
            ? 'Lookup temporarily unavailable'
            : (phonetic ? `${phonetic}<br>${definition || 'Not found in vocabulary'}` : definition);
        showWordPopup(popupWord, rect.left + window.scrollX, rect.top + window.scrollY, popupDefinition);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (contentDiv) {
        ...
        contentDiv.addEventListener('dblclick', handleWordLookupFromSelection);
    }
});
